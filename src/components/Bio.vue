<script setup lang="ts">

</script>

<template>
  <div class="card">
    <h1> 🤔 Who am I? </h1>

    <div class="text-container">
      <p>I'm a 16 y.o. high school student and junior backend developer. </p>
      <p>Tried applying to <a href="https://education.vk.company/centrum/internship">VK Internship</a>.</p>
      <p>Finalist in <a href="https://prodcontest.ru">Prod contest 2024</a>.</p>
      <p>Took part in <a href="https://cs.hse.ru/se/2024/">Summer School of Software Engineering at HSE</a>.</p>
      <p>Worked on backend for <a href="https://middle-app.ru">Middle</a>. And <a href="https://t.me/TonOldy_bot">Oldy</a> mini app for Telegram.</p>
    </div>

    <h1> 💻 My Stack </h1>
    <div class="text-container">
      <p> - C# (ASP.NET, Entity Framework, Telegram.Bot, TDLib, Discord.NET) </p>
      <p> - Go (Fiber, Gorm, telebot) </p>
      <p> - TypeScript (simple openapi clients) </p>
      <p> - VueJS (Router, Pinia, Vuetify) </p>
      <br>
      <p> - Linux (Arch, Fedora, Debian/Ubuntu) + Caddy/Traefik + Docker (Compose) </p>
      <p> - GitHub actions </p>
    </div>
  </div>
</template>

<style scoped>
</style>
